...
with col1:
    ...
    # 添加水質監測站標記並設置自定義圖標
    folium.GeoJson(water_quality_stations_url, name="Water Quality Stations", 
                   marker=folium.Marker(icon=folium.Icon(icon_url=water_icon_url, icon_size=(30, 30)))).add_to(m)

    # 添加釣魚點標記並設置自定義圖標
    folium.GeoJson(fishing_spots_url, name="Fishing Spots", 
                   marker=folium.Marker(icon=folium.Icon(icon_url=fish_icon_url, icon_size=(30, 30)))).add_to(m)
